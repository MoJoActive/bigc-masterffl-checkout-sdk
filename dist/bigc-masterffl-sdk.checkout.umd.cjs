(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.MasterFFLCheckout = {}, global.React, global.ReactDOM));
  ...
  const MasterFFLContext = require$$0.createContext({});
  const useMasterFFL = () => {
    const context = require$$0.useContext(MasterFFLContext);
    if (!context) {
  ...
  const MasterFFL = ({ checkoutContext, getCheckoutStepStatuses }) => {
    const { checkoutService, checkoutState } = require$$0.useContext(checkoutContext);
    const storeProfile = require$$0.useMemo(() => checkoutState.data.getConfig()?.storeProfile, [checkoutState]);
    const checkoutId = require$$0.useMemo(() => checkoutState.data.getCheckout()?.id, [checkoutState]);
    const storeHash = require$$0.useMemo(() => storeProfile?.storeHash, [storeProfile]);
    ...
    const [selectedDealer, setSelectedDealer] = require$$0.useState(null);
    const [isFFL, setIsFFL] = require$$0.useState(false);
    const [isSuppressor, setIsSuppressor] = require$$0.useState(false);
    require$$0.useEffect(() => {
    ...
    const handleSaveDealer = require$$0.useCallback(
      async (dealer) => {
    ...
    const steps = require$$0.useMemo(() => getCheckoutStepStatuses(checkoutState), [checkoutState]);
    require$$0.useEffect(() => {
    ...
    const handleInit = require$$0.useCallback(async () => {
  ...
  const MasterFFLForm = () => {
    ...
    const [errors, setErrors] = require$$0.useState({ postalCode: "", acceptTerms: "" });
    const observerRef = require$$0.useRef(null);
    const buttonRef = require$$0.useRef(null);
    const acceptTermsRef = require$$0.useRef(false);
    const selectedDealerRef = require$$0.useRef(false);
    require$$0.useEffect(() => {
    ...
    const applyDisabledState = require$$0.useCallback(() => {
  ...
  const MasterFFLModal = () => {
    ...
    const dealerSelectionCallback = require$$0.useCallback(
      async (dealer) => {
        if (!dealer) return;
  ...
  exports.MasterFFL = MasterFFL;
  exports.useMasterFFL = useMasterFFL;
